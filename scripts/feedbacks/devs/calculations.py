...
class FeedbackAnalysis:
    def __init__(self, uptake, CO2, temp, time='year', sink="Earth_Land",
    time_range=None):
        ...
        C = pd.read_csv(Cfname, index_col='Year')['CO2']
        T = xr.open_dataset(Tfname)

        U_time = (pd
                    .to_datetime([time.year for time in U.time.values],
                                format='%Y'
                                )
                    .year
                 )
        ...
        self.CO2 = C.loc[slice(start, stop)]
...
df = FeedbackAnalysis(uptake=('inversions', 'CAMS'), CO2='raw', temp='CRUTEM',
                      time='year', sink="Earth_Land",
                      time_range = slice("1989", "2007"))

df.model.summary()
df.confidence_intervals('CO2', 0.05)
df.feedback_output(OUTPUT_DIR + 'feedbacks/test.txt')

start, stop = "1999", "2007"

CC = C.loc[slice(start, stop)].values
UU = U.sel(time=slice(start, stop))["Earth_Land"].values
TT = T.sel(time=slice(start, stop))["Earth"].values

len(CC), len(UU), len(TT)
pd.date_range(start, stop, freq="Y", closed=None)
pd.date_range(start, stop, freq="Y", closed='left')

pd.date_range(*(pd.to_datetime([start, stop]) + pd.offsets.YearEnd()), freq='Y').strftime("%Y")
...
df = pd.DataFrame(data = {
                  ...
                  index = pd.date_range(start, stop, freq="Y")
                 )
