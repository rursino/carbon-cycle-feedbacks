...
class Analysis:
    ...
    def __init__(self, variable):
        """Read budget.csv and convert the columns into single arrays assigned to a variable."""
    ...
    def rolling_trend(self, window_size=25, plot=False, include_pearson=False):
        ...
        roll_df = pd.DataFrame({f"{window_size}-year trend slope": roll_vals}, index=df[:-window_size].index)
        ...
        if include_pearson:
            r_df = pd.DataFrame({"r-values of trends": r_vals}, index=df[:-window_size].index)
            return roll_df, r_df
        else:
            return roll_df
